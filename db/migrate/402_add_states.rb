...
class AddStates < ActiveRecord::Migration[4.2]
    ...
    reversible do |dir|
      dir.up do
        execute <<-SQL.strip_heredoc
          ...
          (state IN (#{config_default[:constants][:STATES][:JOB].map{|s|"'#{s}'"}.join(', ')}));
          ...
          (state IN (#{config_default[:constants][:STATES][:TASK].map{|s|"'#{s}'"}.join(', ')}));
          ...
          (state IN (#{config_default[:constants][:STATES][:TRIAL].map{|s|"'#{s}'"}.join(', ')}));
          ...
          (state IN (#{config_default[:constants][:STATES][:SCRIPT].map{|s|"'#{s}'"}.join(', ')}));
